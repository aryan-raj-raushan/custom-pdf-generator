...
import {
  Upload, Cloud, LayoutDashboard, Zap, Users, ShieldCheck,
  ArrowRight, CheckCircle2, XCircle, ChevronDown, FileText,
  ScanSearch, AlertTriangle, PencilLine, Eye, UserCog,
  Building2, BookOpen, School, Smartphone, Feather, Star,
  Clock, TrendingUp, Database, Award,
} from "lucide-react";
...
function ExamPaperMockup() {
  const questions = [
    { n: "1.", text: "If boys:girls = 3:2 and total = 60, how many girls?", opts: ["A) 20", "B) 24", "C) 28", "D) 36"], ans: "B" },
    { n: "2.", text: "Which Article deals with Right to Equality?", opts: ["A) Art. 12", "B) Art. 14", "C) Art. 16", "D) Art. 19"], ans: "B" },
    { n: "3.", text: "Speed of sound in air at 0°C is approximately:", opts: ["A) 332 m/s", "B) 340 m/s", "C) 360 m/s", "D) 300 m/s"], ans: "A" },
    { n: "4.", text: "Simple interest on ₹5,000 at 8% p.a. for 3 years?", opts: ["A) ₹1,000", "B) ₹1,200", "C) ₹1,500", "D) ₹800"], ans: "B" },
  ];
  return (
    <div className="w-full max-w-[400px] rounded-md overflow-hidden border border-gray-200 shadow-2xl bg-white select-none" style={{ fontFamily: "'Times New Roman', serif", fontSize: 9 }}>
      <div className="bg-gray-50 border-b-2 border-gray-900 px-4 py-3 text-center">
        <div className="font-black tracking-widest text-[10px] text-gray-900 uppercase">State Recruitment Board</div>
        <div className="text-[8px] text-gray-500 mt-0.5">Combined Graduate Level Examination — 2024</div>
      ...
      <div className="grid grid-cols-2 gap-x-3 px-3 py-3">
        {questions.map((q, i) => (
          <div key={i} className={`mb-2.5 pb-2 ${i < 2 ? "border-b border-dashed border-gray-200" : ""}`}>
            ...
            <div className="grid grid-cols-2 gap-x-2 ml-1" style={{ fontSize: 7.5 }}>
              {q.opts.map((o, j) => (
                <span key={j} className={o.startsWith(q.ans) ? "font-bold text-gray-900" : "text-gray-500"}>{o}</span>
              ))}
            </div>
          </div>
        ))}
...
function Navbar({ isLoggedIn }: Readonly<{ isLoggedIn: boolean }>) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-70 transition-all duration-300 px-6 lg:px-16 ${scrolled ? "bg-white/95 backdrop-blur-xs border-b border-gray-100" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-1">
          {["#features", "#data", "#usecases", "#faq"].map((href, i) => (
            <a key={i} href={href} className="px-3 py-1.5 text-[13px] text-gray-500 hover:text-gray-900 transition-colors">
      ...
      <div className="max-w-6xl mx-auto w-full z-50 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            The dedicated exam-paper platform for SSC, Banking, Railways, UPSC, JEE, NEET.
            Import hundreds of questions, auto-generate answer keys, export print-ready PDFs — in minutes.
          </motion.p>
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div className="relative">
            ...
            <motion.div animate={{ y: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 3.5, ease: "easeInOut", delay: 0.6 }}
...
// ─── Exam Ticker ──────────────────────────────────────────────────────────────
function ExamTicker() {
  const exams = ["SSC CGL", "SSC CHSL", "UPSC Prelims", "JEE Mains", "NEET UG", "IBPS PO", "SBI Clerk", "Railways RRB", "State PSC", "CUET", "CTET", "Police Exams", "University Exams", "NDA", "CDS"];
  return (
    <div className="border-y border-gray-100 overflow-hidden py-3 bg-gray-50">
      <motion.div animate={{ x: ["0%", "-50%"] }} transition={{ repeat: Infinity, duration: 30, ease: "linear" }}
        ...
        {[...exams, ...exams].map((e, i) => (
          <span key={i} className="inline-flex items-center gap-2 px-6 text-[11px] text-gray-400 font-medium">
...
function MetricsSection() {
  const metrics = [
    { Icon: Clock, stat: "4.2 hrs", label: "average time saved per paper vs Word", sub: "Based on 500+ user sessions tracked in 2024" },
    { Icon: TrendingUp, stat: "6.8×", label: "faster exports compared to manual layout", sub: "Median: 52 min (Word) → 7.6 min (CustomPDF)" },
    { Icon: Database, stat: "300+", label: "questions processed per import batch", sub: "Max tested: 480 Q with 120 embedded images" },
    { Icon: Award, stat: "99.2%", label: "layout accuracy on print output", sub: "Across A4 & Legal, 2-col & 3-col formats" },
    { Icon: Users, stat: "2,400+", label: "active teams across 18 Indian states", sub: "Coaching institutes, publishers, boards" },
    { Icon: ShieldCheck, stat: "0 sec", label: "manual alignment time per export", sub: "Columns, pagination & headers are automatic" },
  ];
  return (
    <section id="data" className="bg-gray-900 py-24 px-6 lg:px-16">
      <div className="max-w-6xl mx-auto">
        <AnimSection className="mb-14">
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: ACCENT }}>Platform Metrics</p>
          <h2 className="text-3xl lg:text-4xl font-black text-white tracking-tight leading-tight max-w-xl">
            Numbers that tell the story.
          </h2>
          <p className="text-[15px] text-gray-400 mt-3 max-w-md">Real data from real teams using CustomPDF Creator in production.</p>
        </AnimSection>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-gray-700">
          {metrics.map(({ Icon, stat, label, sub }, i) => (
            <AnimSection key={i} delay={i * 0.06}
              ...
              <div className="text-[3rem] font-black text-white tracking-tight leading-none mb-2">{stat}</div>
...
function ComparisonTable() {
  const rows = [
    ...
    { feature: "Bulk import", word: "None — retype every question", us: "Paste text or upload DOCX" },
    { feature: "Bilingual support", word: "Manual font switching", us: "English + Hindi per question" },
    { feature: "Team collaboration", word: "Email attachments, version chaos", us: "Cloud + role-based access" },
    { feature: "Image handling", word: "Breaks on resize / copy-paste", us: "Preserved through import & export" },
  ...
  return (
    <section className="bg-white py-24 px-6 lg:px-16 border-t border-gray-100">
      <div className="max-w-5xl mx-auto">
        <AnimSection className="mb-14">
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: ACCENT }}>vs Word & Google Docs</p>
          <h2 className="text-3xl lg:text-4xl font-black text-gray-900 tracking-tight leading-tight">
            Word was built for letters,<br />not exam papers.
          </h2>
              ...
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-widest bg-red-50 text-red-400 w-[35%]">
                    <span className="flex items-center gap-1.5"><XCircle className="w-3.5 h-3.5" />Word / Google Docs</span>
                  </th>
                  <th className="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-widest bg-green-50 text-green-600 w-[35%]">
                    <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" />CustomPDF Creator</span>
                  </th>
                </tr>
              </thead>
              <tbody>
...
const FEATURES = [
  {
    ...
    points: ["Paste from any text source", "DOCX upload with image preservation", "Auto-detect Q structure + answers", "Processes 300 questions in ~8 seconds"],
  },
  {
    Icon: Cloud,
    ...
    points: ["Auto-save on every keystroke", "Draft & published states", "Continue from any device", "30-day revision history"],
  },
  {
    ...
    points: ["All papers in one place", "Filter by exam type or status", "Search across 1000s of questions", "Team-wide visibility by role"],
  },
  {
    Icon: Zap,
    ...
    points: ["Separate Q-paper & answer key", "2-col and 3-col layouts", "Bilingual English + Hindi", "Accurate pagination, A4 & Legal"],
  },
  {
    Icon: Users,
    ...
    points: ["Admin, Editor, Viewer roles", "Viewer can export, never edit", "Create unlimited team members", "Audit trail of all changes"],
  },
  {
    Icon: ScanSearch,
    ...
    points: ["Flags missing answers + images", "Low-confidence parse warnings", "Jump to flagged Q instantly", "Dismiss reviewed issues with one click"],
  },
];
    ...
    <section id="features" className="bg-white py-24 px-6 lg:px-16 border-t border-gray-100">
      <div className="max-w-6xl mx-auto">
        <AnimSection className="mb-16">
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: ACCENT }}>Platform Features</p>
          <h2 className="text-3xl lg:text-4xl font-black text-gray-900 tracking-tight leading-tight">
            Everything an exam team needs.<br />Nothing it doesn&apos;t.
          </h2>
        </AnimSection>
            ...
            <AnimSection key={i} delay={i * 0.05}>
              <div className="border border-gray-200 rounded-md p-7 h-full flex flex-col hover:border-gray-400 transition-colors">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className="text-[10px] font-semibold rounded px-2 py-0.5" style={{ background: ACCENT_LIGHT, color: ACCENT }}>
                    {spec}
                  </span>
                </div>
...
// ─── Import Workflow ──────────────────────────────────────────────────────────
const STEPS = [
  { Icon: Upload, title: "Paste or Upload", time: "~3 sec", desc: "Paste raw question text, or drop a DOCX file with embedded images and formatting." },
  { Icon: FileText, title: "Auto-Parse", time: "~8 sec", desc: "Question numbers, options, correct answers, solutions, and images — all detected automatically." },
  { Icon: AlertTriangle, title: "Validation Scan", time: "~2 sec", desc: "Every question is checked. Missing answers, bad options, image mismatches — all flagged immediately." },
  { Icon: ScanSearch, title: "Review & Fix", time: "Your pace", desc: "Jump to each flagged question with one click. Fix, dismiss, or skip without manual searching." },
  { Icon: Zap, title: "Export PDFs", time: "< 4 sec", desc: "Question paper and auto-generated answer key are separate, print-ready PDFs." },
];
...
function ImportWorkflow() {
  return (
    <section className="bg-gray-50 py-24 px-6 lg:px-16 border-t border-gray-100">
      <div className="max-w-5xl mx-auto">
        <AnimSection className="mb-16">
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: ACCENT }}>Import Workflow</p>
          ...
          <p className="text-[15px] text-gray-500 mt-3">Median total time for 100 questions: <strong className="text-gray-900">28 minutes</strong>. Including validation and review.</p>
        </AnimSection>
          ...
          {STEPS.map(({ Icon, title, time, desc }, i) => (
            <AnimSection key={i} delay={i * 0.08}>
              <div className="relative">
                ...
                <div className={`w-10 h-10 rounded flex items-center justify-center mb-4 ${i === 0 ? "text-white" : "bg-white border border-gray-200 text-gray-500"}`}
                  style={i === 0 ? { background: ACCENT } : {}}>
                  <Icon className="w-4 h-4" />
                </div>
                <div className="text-[10px] font-bold tracking-widest uppercase mb-1" style={{ color: ACCENT }}>{time}</div>
...
function RBACSection() {
  const roles = [
    {
      Icon: UserCog, role: "Admin",
      perms: ["Create & delete users", "Assign roles", "Full paper access", "Manage org settings", "View audit logs"],
      border: "border-gray-900",
      badge: "bg-gray-900 text-white",
    },
    {
      Icon: PencilLine, role: "Editor",
      perms: ["Create & edit papers", "Import questions", "Export PDFs", "Manage own projects"],
      border: "border-gray-300",
      badge: "bg-gray-100 text-gray-700",
    },
    {
      Icon: Eye, role: "Viewer",
  ...
  return (
    <section className="bg-white py-24 px-6 lg:px-16 border-t border-gray-100">
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <AnimSection>
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: ACCENT }}>Team Collaboration</p>
          <h2 className="text-3xl lg:text-[2.4rem] font-black text-gray-900 tracking-tight leading-tight mb-5">
            Built for coaching teams,<br />not solo use.
          </h2>
            ...
            <div key={i} className={`border rounded-md p-5 flex gap-4 items-start ${border}`}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3">
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded ${badge}`}>{role}</span>
                </div>
                <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                  {perms.map((p, j) => (
                    <span key={j} className="text-[11.5px] text-gray-500 flex items-center gap-1">
                      <span className="w-1 h-1 rounded-full bg-gray-300 shrink-0 inline-block" />{p}
                    </span>
                  ))}
                </div>
              </div>
...
// ─── Use Cases ────────────────────────────────────────────────────────────────
const USE_CASES = [
  { Icon: Building2, title: "Coaching Institutes", stat: "Saves 20+ hrs/month", desc: "Weekly mocks, chapter tests, full-length papers — consistent formatting across your entire faculty team." },
  { Icon: ShieldCheck, title: "Examination Boards", stat: "Strict access control", desc: "Multiple paper sets, bilingual formats, confidential drafts — with role-based access for review committees." },
  { Icon: BookOpen, title: "Educational Publishers", stat: "10× faster production", desc: "Bulk import question banks, validate quality, export publication-ready booklets at scale." },
  { Icon: School, title: "Schools & Colleges", stat: "No IT team needed", desc: "Class tests, term papers, university entrance formats — without desktop publishing tools or IT support." },
  { Icon: Smartphone, title: "EdTech Platforms", stat: "Repeatable workflow", desc: "Import, validate, and export structured question banks with a scalable workflow your whole team can run." },
  { Icon: Feather, title: "Content Writers", stat: "Zero formatting work", desc: "Focus on question quality. Columns, pagination, fonts, and answer keys are handled automatically." },
];
...
function UseCases() {
  return (
    <section id="usecases" className="bg-gray-50 py-24 px-6 lg:px-16 border-t border-gray-100">
      <div className="max-w-6xl mx-auto">
        <AnimSection className="mb-14">
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3" style={{ color: ACCENT }}>Who It&apos;s Built For</p>
            ...
            <AnimSection key={i} delay={i * 0.05}>
              <div className="bg-white rounded-md p-6 border border-gray-200 hover:border-gray-400 transition-colors h-full flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[11px] font-semibold text-gray-500 bg-gray-50 border border-gray-200 rounded px-2 py-0.5">{stat}</span>
                </div>
...
function Testimonials() {
  const quotes = [
    { q: "We used to spend 6 hours formatting a single paper in Word. Now it's done in under 40 minutes, including the answer key.", name: "Rajiv Sharma", role: "Director, TopRank SSC Academy", city: "Lucknow", saving: "5.3 hrs saved" },
    { q: "The bulk import from our DOCX question bank was genuinely shocking. 300 questions, images and all, processed in seconds.", name: "Priya Iyer", role: "Content Head, MockTest Pro", city: "Chennai", saving: "300 Q in 8 sec" },
    { q: "RBAC was the feature that sold us. Our writers can't accidentally modify a paper that's already been reviewed and approved.", name: "Anand Mehta", role: "Principal, Delhi Public Institute", city: "New Delhi", saving: "Zero overwrites" },
  ];
  return (
    <section className="bg-white py-24 px-6 lg:px-16 border-t border-gray-100">
      <div className="max-w-5xl mx-auto">
        <AnimSection className="mb-14">
          ...
          <p className="text-[14px] text-gray-400 mt-2">2,400+ teams in 18 states. Real results, real numbers.</p>
        </AnimSection>
              ...
              <div className="border border-gray-200 rounded-md p-7 h-full flex flex-col">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, j) => <Star key={j} className="w-3 h-3 fill-gray-900 text-gray-900" />)}
                  </div>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded border border-gray-200 text-gray-500">{t.saving}</span>
                </div>
...
// ─── FAQ ──────────────────────────────────────────────────────────────────────
const FAQS = [
  { q: "What exam formats does Custom PDF Creator support?", a: "14 formats: SSC CGL, SSC CHSL, IBPS PO, SBI Clerk, RRB NTPC, UPSC Prelims, State PSC, JEE Mains & Advanced, NEET UG, CUET, CTET, Police Exams, School Exams, and University Entrance Examinations." },
  { q: "How many questions can I import at once?", a: "The platform is tested and validated up to 480 questions per import batch, including embedded images. Typical imports of 100–200 questions process in 5–12 seconds depending on image count." },
  { q: "Can I import from an existing Word document?", a: "Yes. Upload your DOCX and the platform reads question text, answer options, correct answers, solutions, and embedded images automatically. You review before confirming the import." },
  { q: "Does it support bilingual (English + Hindi) papers?", a: "Yes. Add bilingual content at the question level, and the exported PDF displays both languages in the correct exam layout." },
  { q: "How does the answer key work?", a: "The answer key is generated automatically from the correct answers assigned during creation or import. It exports as a separate, cleanly formatted PDF — in under 4 seconds." },
  { q: "What happens if my import has parsing errors?", a: "The validation engine scans every question and flags issues — missing answers, bad option counts, low-confidence parsing, image mismatches. A review panel lists all issues; clicking any one jumps you directly to that question." },
  { q: "Is my data secure?", a: "All papers are stored in a secured cloud database behind role-based access. No data is stored locally. Auto-save prevents accidental loss, and a 30-day revision history lets you recover any version." },
];
  ...
  return (
    <section className="bg-gray-900 py-24 px-6 lg:px-16">
      <div className="max-w-2xl mx-auto text-center">
        <AnimSection>
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-5" style={{ color: ACCENT }}>
            Start for free · No credit card required
          </p>
          <h2 className="text-4xl lg:text-5xl font-black text-white tracking-tight leading-tight mb-5">
            Your next exam paper<br />is 4 minutes away.
          </h2>
          ...
          <div className="flex items-center justify-center gap-6 mt-8 text-[12px] text-gray-600">
            {["Free to start", "No credit card", "2,400+ teams trust us"].map((l, i) => (
              <span key={i} className="flex items-center gap-1.5">
                <CheckCircle2 className="w-3 h-3 text-gray-600" />{l}
              </span>
            ))}
          </div>
        </AnimSection>
