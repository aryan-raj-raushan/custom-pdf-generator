...
const SOLUTION_START_RE =
    /^\s*(?:Solution|Sol|व्याख्या|हल|स्पष्टीकरण)\s*[:\-]\s*(.*)$/iu;
...
const EMPTY_OR_IMAGE_OPTION_RE =
    /^\s*(image|img|चित्र|figure|आकृति)?\s*[-—]?\s*$/iu;
...
function splitIntoBlocks(text: string): {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isSequential =
            candidateNumber !== null && candidateNumber === lastSourceIndex + 1;
        ...
        if (mode === "options" && current.optionLines.length > 0) {
            current.optionLines[current.optionLines.length - 1].text +=
                " " + line.trim();
            continue;
        }
...
function buildQuestion(
    ...
    if (isMcq) {
        ...
        options = block.optionLines.map((o) => {
            ...
            const resolvedImage = images && optImageIndices.length > 0
                ? images.get(optImageIndices[0])
                : undefined;

            const isImageLike =
                EMPTY_OR_IMAGE_OPTION_RE.test(optTextCleaned) ||
                IMAGE_CUE_RE.test(o.text);
            ...
            return {
                ...
                isCorrect: block.answerLetter
                    ? o.letter === block.answerLetter
                    : false,
            };
        });

        if (block.answerLetter) {
            const matched = block.optionLines.some(
                (o) => o.letter === block.answerLetter,
            );
            if (!matched) {
                flags.push({
    ...
    const resolvedQuestionImage = images && questionImageIndices.length > 0
        ? images.get(questionImageIndices[0])
        : undefined;
    ...
    const solutionJoined = block.solutionLines
        .join(" ")
        .replace(/\s+/g, " ")
        .trim();
    ...
    if (!textEn && !textHi) {
        flags.push({
            type: "low_confidence",
            message:
                "Question text came out empty — the parser likely misread this block's boundaries.",
        });
    }
