...
import { MAX_PROJECTS_PER_OWNER, canEditWorkspace, getOwnerUsername } from '@/lib/access';
...
export async function POST(req: NextRequest) {
  ...
  if (!canEditWorkspace(session)) {
    return NextResponse.json({ error: 'View-only users cannot create papers' }, { status: 403 });
  }
  ...
  if (!name || !body.paper) {
    return NextResponse.json({ error: 'Name and paper are required' }, { status: 400 });
  }
