...
export async function GET(_req: NextRequest, ctx: RouteContext<'/api/papers/[id]'>) {
  ...
  const doc = await db.collection<PaperDocument>('papers').findOne({ _id: oid, ownerUsername });
...
export async function PUT(req: NextRequest, ctx: RouteContext<'/api/papers/[id]'>) {
  const { id } = await ctx.params;
  const session = await getServerSession(authOptions);
  if (!session) {
    return NextResponse.json({ error: 'Unauthorised' }, { status: 401 });
  ...
  if (!canEditWorkspace(session)) {
    return NextResponse.json({ error: 'View-only users cannot update papers' }, { status: 403 });
  }
  ...
  if (body.paper !== undefined) {
    ...
    const totalQuestions = body.paper.sections.reduce((sum, s) => sum + s.questions.length, 0);
...
export async function DELETE(_req: NextRequest, ctx: RouteContext<'/api/papers/[id]'>) {
  const { id } = await ctx.params;
  const session = await getServerSession(authOptions);
  if (!session) {
    return NextResponse.json({ error: 'Unauthorised' }, { status: 401 });
  ...
  if (!canEditWorkspace(session)) {
    return NextResponse.json({ error: 'View-only users cannot delete papers' }, { status: 403 });
  }
